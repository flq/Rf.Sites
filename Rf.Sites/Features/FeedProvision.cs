...
{
    [HasActions]
    public class FeedProvision
    {
        private readonly IRepository<Content> _contents;
        private readonly FeedSetup _feedSetup;
        private readonly int _itemsToTake;

        public FeedProvision(IRepository<Content> contents, IUrlRegistry registry, ServerVariables vars, PaginationSettings pagination, SiteSettings site)
        {
            _contents = contents;
        ...
        public IStreamOutput RssFeed(InputFeedTag tag)
        {
            _feedSetup.FeedType = FeedType.Rss;
            return ContentFeed(tag.Tag);
        }

        public IStreamOutput AtomFeed(InputFeedTag tag)
        {
            _feedSetup.FeedType = FeedType.Atom;
            return ContentFeed(tag.Tag);
        }

        private IStreamOutput ContentFeed(string tag)
        {
            AppendSubtitle(tag);
