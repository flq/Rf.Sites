...
namespace Rf.Sites.Frame
{
    public class DynamicJson : DynamicObject
    {
        ...
        public override bool TryConvert(ConvertBinder binder, out object result)
        {
            ...
            {
                var ie = (IsArray)
                    ? xml.Elements().Select<XElement, object>(x => ToValue(x))
                    : xml.Elements().Select<XElement, object>(x => (dynamic)new KeyValuePair<string, object>(x.Name.LocalName, ToValue(x)));
